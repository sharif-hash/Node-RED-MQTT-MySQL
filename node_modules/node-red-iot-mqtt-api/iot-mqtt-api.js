...
module.exports = function (RED) {
    ...
    function doPublish(node, msg, done) {
        try {
            ...
            if (node.topicCategory) { msg.topicCategory = node.topicCategory; }
            ...
            if (node.topicCategory === "eventUp") {
                msg.eventName = node.eventName;
            } else if (node.topicCategory === "serviceInvoke") {
                msg.serviceName = node.serviceName;
            }
    ...
    function MQTTBrokerNode(n) {
        ...
        node.deviceName = n.deviceName;
        node.productKey = n.productKey;
        ...
        node.setOptions = function (opts, init) {
            ...
            setIfHasProperty({
                ...opts,
                clientid: `${n.productKey}_${n.deviceName}_${n.model}`
            }, node, "clientid", init);
            setIfHasProperty(opts, node, "model", init);
            ...
            // node.options.clientId = `${n.productKey}_${n.deviceName}_${n.model}`;

            node.username = n.deviceName
            // mqttPassword: md5(产品密钥,mqttClientId)
            node.password = crypto.createHash('md5').update(`${n.secrete}${node.clientid}`).digest('hex');
            ...
            if (!node.cleansession && !node.model) {
                node.cleansession = true;
            ...
            if (node.canConnect()) {
                ...
                try {
                    ...
                    node._clientOn('error', function (error) {
                        node.log(RED._("mqtt.state.error-connect", { broker: (node.clientid ? node.clientid + "@" : "") + node.brokerurl, error: error.message }));
                    });
                } catch (err) {
                    console.log(err);
                }
        ...
        node.publish = function (msg, done) {
            if (node.connected) {
                ...
                if (topicOK) {
                    ...
                    // msg.topic = parseTopic(msg.topic)
                    // /sys/{productKey}/{deviceName}/s/event/property/post
                    // msg.topic = `/sys/${n.productKey}/${n.deviceName}/s/event/property/post`;

                    function generateTopic(msg, n) {
                        let topic;
                        const productKey = n.productKey
                        const deviceName = n.deviceName
                        switch (msg.topicCategory) {
                            // 事件上报
                            case "eventUp":
                                // /sys/{productKey}/{deviceName}/s/event/事件名
                                topic = `/sys/${productKey}/${deviceName}/s/event/${msg.eventName}`
                                break;

                            // 服务调用
                            case "serviceInvoke":
                                topic = `/sys/${productKey}/${deviceName}/c/service/${msg.serviceName}`
                                break;
                            case "propertySet":
                            ...
                            case "propertyGet":
                                ...
                                break;
                            case "propertyPost":
                                topic = `/sys/${productKey}/${deviceName}/c/event/property/post`
                                break;
                            case "deviceConfigGet":
                                topic = `/sys/${productKey}/${deviceName}/s/config/get`
                                break;
                            case "deviceConfigSet":
                    ...
                    function generatePayload(msg, payload) {
                        payload = {
                        ...
                        switch (msg.topicCategory) {
                            case "eventUp":
                                ...
                                payload.method = `thing.event.property.${msg.eventName}`
                                break;
                            case "serviceInvoke":
                                payload.id = generateUUID()
                                payload.method = `thing.service.${msg.serviceName}`
                                break;
                            case "propertySet":
                                payload.id = generateUUID()
                                payload.method = `thing.service.property.set`
                                break;
                            case "propertyGet":
                                payload.id = generateUUID()
                                payload.method = `thing.service.property.get`
                                break;
                            case "propertyPost":
                                payload.id = generateUUID()
                            ...
                            case "deviceConfigGet":
                                payload.id = generateUUID()
                                payload.method = `thing.config.get`
                                break;
                            case "deviceConfigSet":
                                payload.id = generateUUID()
                                payload.method = `thing.config.set`
                                break;
                        }
                        
                        return JSON.stringify(payload)
                    }
                    const topic = generateTopic(msg, n)
                    const payload = generatePayload(msg, msg.payload)
                    node.client.publish(topic, payload, options, function (err) {
                        if (done) {
                            done(err)
                        } else if (err) {
                            node.error(err, msg)
    ...
    RED.nodes.registerType("iot-mqtt-broker", MQTTBrokerNode, {
        credentials: {
            user: { type: "text" },
    ...
    function MQTTInNode(n) {
        ...
        node.topicCategory = n.topicCategory
        function generateTopic(category) {
            if (!node.brokerConn || !node.brokerConn.productKey || !node.brokerConn.deviceName) {
                node.error("请先配置MQTT Broker")
                return
            }
            let topic;
            const productKey = node.brokerConn.productKey
            const deviceName = node.brokerConn.deviceName
            switch (category) {
                case "online":
                    topic = `/sys/${productKey}/${deviceName}/c/#`
                    break;
                case "eventUp":
                    topic = `/sys/${productKey}/${deviceName}/c/event/${n.eventName}_reply`
                    break;
                case "serviceInvoke":
                    topic = `/sys/${productKey}/${deviceName}/s/service/${n.serviceName}_reply`
                    break;
                case "propertySet":
        ...
        node.topic = generateTopic(node.topicCategory)
        node.dynamicSubs = {};
        ...
        if (node.brokerConn) {
            ...
            if (node.topic || node.isDynamic) {
                ...
                if (!node.isDynamic) {
                    ...
                    // 设备上线
                    // node.topic = `/sys/${n.productKey}/${n.deviceName}/c/#`
                    // const topic = generateTopic(node.topicCategory)
    ...
    RED.nodes.registerType("iot-mqtt-in", MQTTInNode);
    ...
    function MQTTOutNode(n) {
        ...
        const node = this;
        // node.topic = n.topic;
        // /sys/{productKey}/{deviceName}/c/#
        // node.topic = `/sys/${n.productKey}/${n.deviceName}/c/`;

        node.topicCategory = n.topicCategory;
        node.eventName = n.eventName;
        node.serviceName = n.serviceName;
        // node.topic = generateTopic(n.topicCategory, n)
    ...
    RED.nodes.registerType("iot-mqtt-out", MQTTOutNode);
